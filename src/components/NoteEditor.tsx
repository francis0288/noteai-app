...
import {
  ...
  Palette, Plus, Minus, CheckSquare, AlignLeft, Sparkles,
} from "lucide-react";
import type { Note, Tag, NoteColor, RecurringType, NoteType, NotePayload, NoteAttachment } from "@/types";
import { colorToBg, colorToBgDark, tagColorToClass, plainTextToHtml } from "@/lib/utils";
...
export default function NoteEditor({
  note, allTags, initialFocus = "content", onSave, onUpdate, onDelete, onClose,
  ...
  return (
    <div className="fixed inset-0 bg-black/40 z-50 flex items-start justify-center pt-16 px-4" onClick={handleClose}>
      <div
      ...
      >
        ...
        <div className="flex items-center gap-2 px-4 pt-4">
          <input
            ...
            placeholder="Title"
            className="flex-1 bg-transparent text-gray-800 dark:text-gray-100 font-semibold text-lg placeholder-gray-400 outline-none"
          />
          <button onClick={handleClose} className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 p-1">
            <X size={18} />
          </button>
        </div>
        ...
        <div className="px-4 py-2">
          {type === "text" ? (
            ...
            <div className="space-y-1 min-h-[80px]">
              {checklistItems.map((item, i) => (
                <div key={i} className="flex items-center gap-2">
                  <button onClick={() => toggleCheckItem(i)}>
                    <span className={`text-base ${item.checked ? "text-gray-400" : "text-gray-600 dark:text-gray-300"}`}>
                      {item.checked ? "☑" : "☐"}
                    </span>
                  </button>
                  <input
                    ...
                    className={`flex-1 bg-transparent text-base outline-none ${item.checked ? "line-through text-gray-400" : "text-gray-700 dark:text-gray-200"}`}
              ...
              <button onClick={addCheckItem} className="flex items-center gap-1 text-sm text-gray-400 hover:text-gray-600 mt-1">
        ...
        {/* Media uploader — only when note is saved */}
        {savedNote && (
        ...
        {selectedTags.length > 0 && (
          <div className="px-4 pb-2 flex flex-wrap gap-1">
            {selectedTags.map(tag => (
              <span key={tag.id} className={`text-sm px-2 py-0.5 rounded-full ${tagColorToClass(tag.color)}`}>{tag.name}</span>
            ))}
          </div>
        )}

        {/* Toolbar */}
        <div className="flex items-center gap-1 px-3 pb-3 border-t border-black/5 dark:border-white/10 pt-2">
          ...
          {savedNote && (
            <>
              ...
              {onAISuggest && (
                <ToolBtn icon={<Sparkles size={16} />} title="AI Suggest" onClick={() => onAISuggest(savedNote.id)} />
              )}
          ...
          <button onClick={handleClose} className="text-sm px-3 py-1 rounded-md text-gray-600 dark:text-gray-300 hover:bg-black/5 dark:hover:bg-white/10 font-medium">
            Close
          </button>
        </div>
        ...
        {panel && (
          <div className="border-t border-black/10 dark:border-white/10">
            ...
            {panel === "tag" && (
              <TagPicker allTags={allTags} selectedIds={tagIds}
                onChange={ids => { setTagIds(ids); markDirty(); }} onCreateTag={onCreateTag} />
            )}
            {panel === "reminder" && savedNote && (
              <ReminderPicker reminders={reminders} noteId={savedNote.id}
                onAdd={(dt, rec) => onAddReminder(savedNote.id, dt, rec)} onDelete={onDeleteReminder} />
            )}
            {panel === "reminder" && !savedNote && (
              <p className="text-sm text-gray-400 p-3">Save the note first to add reminders.</p>
            )}
          </div>
        )}
      </div>
