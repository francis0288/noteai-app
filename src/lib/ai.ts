...
export async function aiSearch(query: string, notes: Note[], apiKey?: string | null): Promise<AISearchResult[]> {
...
export async function aiOrganize(notes: Note[], apiKey?: string | null): Promise<AIOrganizeResult[]> {
  if (notes.length === 0) return [];
  const message = await getClient(apiKey).messages.create({
...
export async function aiGenerateReport(notes: Note[], topic: string, apiKey?: string | null): Promise<AIReport> {
  const message = await getClient(apiKey).messages.create({
...
export async function aiSuggest(note: Note, allNotes: Note[], apiKey?: string | null): Promise<AISuggestion> {
  const message = await getClient(apiKey).messages.create({
...
export async function aiChat(message: string, history: AIChatMessage[], notes: Note[], apiKey?: string | null): Promise<string> {
  const noteSummaries = notes.slice(0, 50).map(noteToSummary).join("\n---\n");
  const systemPrompt = `You are a helpful note assistant. The user has the following notes:\n\n${noteSummaries}\n\nAnswer the user's questions about their notes conversationally and helpfully.`;
